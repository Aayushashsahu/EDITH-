...
## 2024-05-24 - [Avoid Sequential Await Lists]
**Learning:** List comprehensions containing `await` (e.g., `[await func(x) for x in items]`) evaluate sequentially, leading to severe performance bottlenecks for I/O-bound tasks like local LLM embedding operations.
**Action:** Use `asyncio.gather` combined with chunking/batching (e.g., batch size of 5) for concurrent execution to balance speed and system load, rather than sequential evaluation.
