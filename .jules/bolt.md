...
## 2026-06-27 - Concurrency in _embed_all for Local LLM
**Learning:** List comprehensions containing `await` (e.g., `[await func(x) for x in items]`) evaluate sequentially, causing severe performance bottlenecks for I/O-bound tasks like local LLM embedding requests.
**Action:** Replaced sequential execution with `asyncio.gather` combined with chunking (batch size of 5) to balance execution speed without overloading local system resources.
