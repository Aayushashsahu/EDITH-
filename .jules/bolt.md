...
## 2024-08-26 - Preventing asyncio Task Destruction Memory Leaks in __del__
**Learning:** When cleaning up async resources (like cached `aiohttp.ClientSession` instances) in a class's `__del__` method, creating a background task (e.g., `loop.create_task(self.close())`) and assigning it to an instance attribute like `self._close_task` fails to prevent the task from being garbage collected because `self` is already being destroyed. This leads to "Task was destroyed but it is pending!" errors and memory leaks.
**Action:** Assign the background task to a class-level set (e.g., `self.__class__._close_tasks.add(task)`) and attach a callback (`task.add_done_callback(self.__class__._close_tasks.discard)`) to retain a strong global reference until the task completes. This ensures proper cleanup of cached sessions without leaks or warnings.
