...
## 2024-05-24 - [Optimize Memory Embedding Ingestion]
**Learning:** Sequential `[await func(x) for x in list]` in list comprehensions creates severe performance bottlenecks for local LLM text chunk embeddings.
**Action:** Replaced sequential awaits with chunked `asyncio.gather` (batch size 5) to execute concurrently without overloading the local Ollama LLM.
