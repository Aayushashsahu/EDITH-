...
## 2025-05-23 - ARIA Labels for Minimalist Dynamic Modals
**Learning:** When adding inputs to dynamic `innerHTML` modals (e.g., `openTasks`, `openBrain`), structural semantic `<form>` contexts and explicit `<label>` tags are often intentionally omitted to maintain minimalistic styling. This omission creates accessibility issues for screen readers.
**Action:** Always add explicit `aria-label` attributes directly to these `<input>` and `<textarea>` elements for accessibility compliance when semantic labels are not present.
