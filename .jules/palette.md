...
## 2025-01-30 - Accessibility Attributes for Dynamic Containers and Modals
**Learning:** Containers that receive dynamic `innerHTML` updates (like `#notif-zone` or `#chat-log`) need `aria-live="polite"` to announce changes to screen readers. Custom modals (like `#modal`) must include `role="dialog"`, `aria-modal="true"`, and `aria-labelledby` for proper accessibility.
**Action:** Always add these accessibility attributes to dynamic containers and custom modals.
