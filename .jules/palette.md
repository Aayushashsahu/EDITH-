...
## 2026-08-03 - [ARIA Attributes for Dynamic Containers and Modals]
**Learning:** Monolithic vanilla JS apps heavily relying on dynamic `innerHTML` updates need `aria-live="polite"` on containers like notification zones or chat logs to announce changes to screen readers. Custom modals must include `role="dialog"`, `aria-modal="true"`, and `aria-labelledby` for proper accessibility.
**Action:** When working on dynamic `innerHTML` templates lacking semantic context, always ensure containers receiving updates have `aria-live` and custom modals carry the appropriate `dialog` ARIA attributes.
