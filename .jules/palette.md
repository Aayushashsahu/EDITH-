...
## 2026-08-21 - ARIA Attributes for Dynamic Containers and Modals
**Learning:** In vanilla JS applications heavily using `innerHTML` for dynamic updates, screen readers will not announce new content automatically. Furthermore, custom modals lack semantic meaning without explicit ARIA roles.
**Action:** Always add `aria-live="polite"` to containers that receive dynamic text updates (like chat logs and notifications). Always add `role="dialog"`, `aria-modal="true"`, and `aria-labelledby` to custom modal containers to ensure proper identification by assistive technologies.
