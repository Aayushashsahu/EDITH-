...
## 2024-08-25 - [Cross-Site WebSocket Hijacking (CSWSH) in FastAPI]
**Vulnerability:** The WebSocket route in FastAPI (`@app.websocket("/ws")`) did not validate the `Origin` header. Although `CORSMiddleware` was applied to the app, it does not automatically protect WebSocket endpoints from CSWSH, allowing malicious domains to connect and interact with the backend over WebSockets.
**Learning:** In FastAPI/Starlette, `CORSMiddleware` does not automatically protect WebSocket endpoints. To prevent Cross-Site WebSocket Hijacking (CSWSH), WebSocket routes must manually retrieve and validate the `Origin` header.
**Prevention:** Always manually retrieve and validate the `Origin` header (e.g., `ws.headers.get("origin")`) against allowed origins (and ensure proper handling of wildcard '*' configurations) before accepting a WebSocket connection.
