...
## 2024-05-18 - FastAPI WebSocket CSWSH Vulnerability
**Vulnerability:** Cross-Site WebSocket Hijacking (CSWSH) on the `/ws` endpoint due to missing origin validation.
**Learning:** FastAPI's `CORSMiddleware` does NOT automatically protect WebSocket endpoints. It only applies to REST routes, leaving WebSockets vulnerable to cross-site hijacking if relying solely on the middleware.
**Prevention:** Always manually retrieve and validate the `Origin` header (e.g. `ws.headers.get("origin")`) against allowed origins in the WebSocket route before accepting the connection, accounting for wildcard (`*`) configurations.
