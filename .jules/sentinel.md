...
## 2024-08-25 - [Cross-Site WebSocket Hijacking (CSWSH) leading to RCE]
**Vulnerability:** The FastAPI `ws_endpoint` for WebSocket connections did not check the `Origin` header. Starlette's `CORSMiddleware` (which FastAPI uses) explicitly ignores WebSocket upgrades, meaning they are completely unprotected from cross-origin requests by default. Since the app relies on an optional `API_KEY` (disabled by default), any malicious website a user visits could open a WebSocket to `ws://localhost:8888/ws` and execute shell commands on the user's host machine.
**Learning:** WebSockets do not respect the Same-Origin Policy (SOP) by default and rely entirely on the server to validate the `Origin` header. FastAPI/Starlette's CORS middleware does not protect WebSocket routes.
**Prevention:** Always manually retrieve and validate the `Origin` header (`ws.headers.get("origin")`) against an allowlist of trusted origins (like `ALLOWED_ORIGINS`) inside the WebSocket endpoint handler before calling `await ws.accept()`.
