...
## 2024-05-24 - Cross-Site WebSocket Hijacking (CSWSH) in FastAPI
**Vulnerability:** The FastAPI application was vulnerable to Cross-Site WebSocket Hijacking (CSWSH). The `ws_endpoint` WebSocket route was accepting connections without validating the `Origin` header.
**Learning:** `CORSMiddleware` in FastAPI/Starlette only protects HTTP REST endpoints. It does not automatically protect WebSocket endpoints, which means any website could potentially open a WebSocket connection to the server on behalf of a user and steal or spoof data.
**Prevention:** Always manually retrieve and validate the `Origin` header (e.g., `ws.headers.get("origin")`) against a list of allowed origins (e.g., `ALLOWED_ORIGINS`), ensuring to handle wildcard (`*`) configurations properly, before awaiting `ws.accept()` in WebSocket endpoints.
