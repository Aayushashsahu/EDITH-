...
class SecondBrain:
    ...
    async def _embed_all(self, texts: list) -> list:
        # Bolt: Avoid sequential list comprehensions for I/O bound tasks
        # Using asyncio.gather with batching (size=5) to speed up concurrent execution
        # without overloading the local Ollama LLM embeddings endpoint
        results = []
        batch_size = 5
