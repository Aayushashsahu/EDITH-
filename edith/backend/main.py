...
async def verify_api_key(request: Request, api_key_header: str = Security(api_key_header)):
    ...
    if api_key_header is not None and secrets.compare_digest(api_key_header, API_KEY):
        return True
    req_api_key = request.query_params.get("api_key")
    if req_api_key is not None and secrets.compare_digest(req_api_key, API_KEY):
        return True
...
async def ws_endpoint(ws: WebSocket):
    req_api_key = ws.query_params.get("api_key")
    if API_KEY and (req_api_key is None or not secrets.compare_digest(req_api_key, API_KEY)):
