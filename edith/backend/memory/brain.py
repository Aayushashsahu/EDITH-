...
class SecondBrain:
    ...
    async def _embed_all(self, texts: list) -> list:
        # Bolt: List comprehensions with `await` evaluate sequentially.
        # We chunk with `asyncio.gather` and small batches (e.g. 5) to
        # balance embedding speed without overloading local Ollama LLM.
        results = []
        batch_size = 5
