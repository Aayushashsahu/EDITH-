...
class SecondBrain:
    ...
    async def _embed_all(self, texts: list) -> list:
        # Evaluate concurrent chunks via asyncio.gather to avoid sequential await bottleneck.
        # Batch size of 5 chosen to prevent overloading the local LLM embedding service.
        results = []
        batch_size = 5
        for i in range(0, len(texts), batch_size):
            batch = texts[i: i + batch_size]
            batch_res = await asyncio.gather(*(self.llm.embed(t) for t in batch))
